...
EOL
            ...
            # 在 StarCCM+ 中运行网格生成 Java 脚本
            "$STARCCM_PATH" -batch "$MESH_SCRIPT_PATH" -np 190 -power "$sim_file"  # 确保网格生成脚本执行
...
EOL
            ...
            "$STARCCM_PATH" -batch "$TEMP_SCRIPT_PATH" -np 190 -power "$sim_file"  # 确保 StarCCM+ 执行

            # 删除临时脚本文件
            rm $TEMP_SCRIPT_PATH

            # 运行仿真
            "$STARCCM_PATH" -batch -np 190 -power  $sim_file  # 运行仿真
