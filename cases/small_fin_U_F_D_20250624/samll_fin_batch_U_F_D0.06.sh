...
# 设置仿真文件路径
SIM_FILE="/share/fandixiaLab/suguangsheng/Zhaoyang/improve_case/near_wall/small_fin_U_F_D_2025_6_24/fin-1.2-0.4-5-11.54o-200-0.2-u0-f1-n1-Y1-wall-0.5_test_remend_fieldfunc_30mm_nomesh_add_all_field.sim"
if [[ ! -f "$SIM_FILE" ]]; then
...
for frequency in "${frequencies[@]}"; do
    for velocity in "${velocities[@]}"; do
        for offset in "${offset_values[@]}"; do
            ...
            "$STARCCM_PATH" -batch altogether.java -np 180 -power "$sim_file"
